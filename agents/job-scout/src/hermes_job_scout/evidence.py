...
def approve_evidence_pack(
    ...
    try:
        destination.relative_to(current.workspace_root)
    except ValueError as exc:
        raise EvidenceApprovalError("approval destination must be in the same workspace") from exc
    if destination == current.path:
        raise EvidenceApprovalError("approval destination cannot replace the Evidence Pack")
...
def _coerce_approval(approval: EvidenceApproval | Path) -> EvidenceApproval:
    if isinstance(approval, Path):
        try:
            raw, _, _, _ = read_private_bytes(approval)
            return EvidenceApproval.model_validate_json(raw)
        except (OSError, ValueError, WorkspaceSafetyError) as exc:
